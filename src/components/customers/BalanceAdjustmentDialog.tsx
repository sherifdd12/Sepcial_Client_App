...
const BalanceAdjustmentDialog = ({ customer, open, onOpenChange, currentBalance, onSuccess }: BalanceAdjustmentDialogProps) => {
    ...
    const handleAdjustment = async () => {
        ...
        try {
            ...
            if (type === "legal_fees") {
                const { error: legalFeeError } = await supabase.from("legal_fees").insert({
                    customer_id: customer.id,
                    amount: adjAmount,
                    notes: notes || "خصم من الرصيد الزائد",
                    status: 'paid',
                    transaction_id: null
                });
                if (legalFeeError) throw legalFeeError;
            }

            toast({
                title: "تم بنجاح",
                description: type === "refund" ? "تم تسجيل عملية الإرجاع بنجاح" : "تم خصم أتعاب المحاماة بنجاح",
                action: type === "legal_fees" ? (
                    <Button variant="outline" size="sm" onClick={() => navigate("/legal-cases")}>
                        عرض سجل الأتعاب
                    </Button>
                ) : undefined
            });
